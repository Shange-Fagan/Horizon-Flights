  ...
  try {
    ...
    while (attempts < maxRetries) {
      ...
      try {
        ...
        const getCabinClassUrlParam = (cabinClass) => {
          const cabinClassMap = {
            "Economy": null,
          ...
          return cabinClassMap[cabinClass] || null;
        };

        const cabinClassParam = getCabinClassUrlParam(cabinClass);
...
&trip=roundtrip`;
        ...
        await page.waitForSelector('[data-test-id="listings"] li', { timeout: 60000 });
          ...
          try {
            const results = await page.evaluate(() => {
              ...
              ticketItems.forEach((item) => {
                const airline = Array.from(item.querySelectorAll('.uitk-text.uitk-type-200.uitk-text-secondary-theme'))[1]?.innerText || "N/A";
                const airlineImg = item.querySelector('img.uitk-mark.uitk-layout-grid-item.uitk-mark-landscape-oriented')?.getAttribute('src') || "N/A";
                ...
                if (button) link = button.getAttribute('data-url') || button.getAttribute('data-href') || link;
                if (link === window.location.href) link = item.closest('a')?.href || link;
                ...
                if (airline !== "N/A" && price !== "N/A" && airlineImg !== "N/A") {
                  flights.push({ airline, airlineImg, departureTime, returnTime, layoverTime, price, link });
                }
              });
            ...
            console.log(`🌍 Scraped ${results.length} flights!`);
            await browser.close();
            return res.json({ success: true, flights: results });

          } catch (scrapeErr) {
            console.warn("⚠️ Failed to evaluate page:", scrapeErr.message);
          }
    ...
    console.error("🚨 Unexpected error scraping flights:", mainError);
